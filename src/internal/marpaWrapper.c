...
#define MARPAWRAPPER_LOG_INFO0(callerfmts)      MARPAWRAPPER_LOG_0(MARPAXML_LOGLEVEL_INFO, MARPAWRAPPERERRORORIGIN_NA, 0, callerfmts)
#define MARPAWRAPPER_LOG_INFOX(callerfmts, ...) MARPAWRAPPER_LOG_X(MARPAXML_LOGLEVEL_INFO, MARPAWRAPPERERRORORIGIN_NA, 0, callerfmts, __VA_ARGS__)
#ifndef MARPAWRAPPER_NTRACE
#define MARPAWRAPPER_LOG_TRACE0(callerfmts)      MARPAWRAPPER_LOG_0(MARPAXML_LOGLEVEL_TRACE, MARPAWRAPPERERRORORIGIN_NA, 0, callerfmts)
#define MARPAWRAPPER_LOG_TRACEX(callerfmts, ...) MARPAWRAPPER_LOG_X(MARPAXML_LOGLEVEL_TRACE, MARPAWRAPPERERRORORIGIN_NA, 0, callerfmts, __VA_ARGS__)
#else
...
#define MARPAWRAPPER_LOG_SYS_ERROR(errorCode, calls) MARPAWRAPPER_LOG_0(MARPAXML_LOGLEVEL_ERROR, MARPAWRAPPERERRORORIGIN_SYSTEM, errorCode, calls);
...
marpaWrapper_t *marpaWrapper_newp(marpaWrapperOption_t *marpaWrapperOptionp) {
  ...
  if (marpaWrapperp == NULL) {
    _marpaWrapper_logExt(marpaWrapperOption.logCallbackp,
  ...
  marpaWrapperp->marpaXmlLogp = marpaXmlLog_newp(marpaWrapperOption.logCallbackp, marpaWrapperOption.logCallbackDatavp, marpaWrapperOption.logLevelWantedi);
...
static C_INLINE void _marpaWrapper_logExt(marpaXmlLogCallback_t     logCallbackp,
  ...
  if (marpaWrapperp != NULL && marpaWrapperp->marpaXmlLogp != NULL) {
...
static C_INLINE void _marpaWrapperStackFailureCallback(genericStack_error_t errorType, int errorNumber, void *failureCallbackUserDatap) {
  ...
  _marpaWrapper_logWrapper(marpaWrapperp, errorType, errorNumber, msgs, MARPAXML_LOGLEVEL_ERROR);
}
...
marpaWrapperBool_t marpaWrapper_optionDefaultb(marpaWrapperOption_t *marpaWrapperOptionp) {
  ...
  marpaWrapperOptionp->logLevelWantedi        = MARPAXML_LOGLEVEL_WARNING;
  marpaWrapperOptionp->logCallbackp           = marpaXmlLog_defaultLogCallback();
...
marpaWrapperBool_t marpaWrapper_r_recognizeb(marpaWrapper_t *marpaWrapperp,
  ...
  if (isLexemebCallbackp == NULL) {
    MARPAWRAPPER_LOG_ERROR0("isLexemebCallbackp must be non-NULL");
    return MARPAWRAPPER_BOOL_FALSE;
  }
  if (lexemeValuebCallbackp == NULL) {
    MARPAWRAPPER_LOG_ERROR0("lexemeValuebCallbackp be non-NULL");
    return MARPAWRAPPER_BOOL_FALSE;
  }
  ...
  while (streamInUtf8_nexti(streamInp, &nexti) == STREAMIN_BOOL_TRUE) {
...
#ifndef MARPAWRAPPER_NTRACE
    {
      ...
      if (marpaWrapper_r_progressb(marpaWrapperp, -2, -1, &nmarpaWrapperProgressi, &marpaWrapperProgresspp) == MARPAWRAPPER_BOOL_TRUE) {
        for (i = 0; i < nmarpaWrapperProgressi; i++) {
          MARPAWRAPPER_LOG_TRACEX("Earley Set Id: %4d, Origin Earley Set Id: %4d, Rule: %10p, Position: %3d: %s",
                                  ...
                                  (ruleToStringCallbackp != NULL && ruleToStringCallbackp(marpaWrapperProgresspp[i]->marpaWrapperRulep, &rules) == MARPAWRAPPER_BOOL_TRUE) ? rules : "");
        }
      }
    }
#endif
    ...
    for (i = 0; i < nMarpaWrapperSymboli; i++) {
      ...
      MARPAWRAPPER_LOG_TRACEX("Checking symbol %s", (symbolToStringCallbackp != NULL && symbolToStringCallbackp(marpaWrapperSymbolpp[i], &symbols) == MARPAWRAPPER_BOOL_TRUE) ? symbols : "");
    ...
    if (maxLengthl > 0) {
      ...
      if (lexemeValuebCallbackp(lexemeValuebCallbackDatavp, streamInp, maxLengthl, &lexemeValuei) == MARPAWRAPPER_BOOL_FALSE) {
        rcb = MARPAWRAPPER_BOOL_FALSE;
        goto recognizebEnd;
      }
      for (i = 0; i < nMarpaWrapperSymboli; i++) {
	...
	goto recognizebEnd;
      ...
      /* A discarded thing then, the isLexemebCallbackp have returned MARPAWRAPPER_BOOL_TRUE, but maxLengthl <= 0: it has in the meantime moved the pointer in the stream */
    }
  }
